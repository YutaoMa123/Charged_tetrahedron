...
context.initialize()
L = 35.0
uc = lattice.unitcell(N=2,a1 = [L,0.,0.], a2 = [0,L,0.], a3 = [0.,0.,L], position = [[L/2,L/2,L/2],[0.,0.,0.]], type_name = ['D','C'])
...
yukawa = md.pair.yukawa(r_cut = 14.0, nlist=nl)
for i in range(len(types)):
	yukawa.pair_coeff.set('C',types[i],epsilon=0.0,kappa=1.0)
yukawa.pair_coeff.set('D','D',epsilon = 16.0,kappa=1./14)
yukawa.pair_coeff.set('D','A',epsilon = -4.0,kappa=1./14)
yukawa.pair_coeff.set('D','B',epsilon = 0.0, kappa=1./14)
yukawa.pair_coeff.set('A','A',epsilon = 1.0, kappa=1./14)
yukawa.pair_coeff.set('A','B',epsilon = 0.0,kappa=1./14)
yukawa.pair_coeff.set('B','B',epsilon = 0.0, kappa= 1./14)
...
lj.pair_coeff.set('B','B',epsilon = 6.0, sigma=sigma[3],r_cut=2.5)
...
wca.pair_coeff.set('A','A',epsilon = 2.0, sigma=1.0,r_cut = 2**(1./6))
wca.pair_coeff.set('A','B',epsilon = np.sqrt(2*6), sigma=1.0,r_cut = 2**(1./6))
...
dumper = dump.gsd(filename = 'randomization.gsd', period = 1e2, group=group.all(), overwrite=True)
